...
std::vector<unsigned char> encrypt_aes_gcm(
    ...
    std::vector<unsigned char> output(
        AES_GCM_IV_LEN + plaintext_len + AES_GCM_TAG_LEN + 16);
...
std::vector<unsigned char> decrypt_aes_gcm(
    ...
    const unsigned char* enc_data = ciphertext + AES_GCM_IV_LEN;
    int enc_len = ciphertext_len - AES_GCM_IV_LEN - AES_GCM_TAG_LEN;
    const unsigned char* tag = ciphertext + AES_GCM_IV_LEN + enc_len;
    ...
    std::vector<unsigned char> plaintext(enc_len + 16);
    int out_len = 0;
