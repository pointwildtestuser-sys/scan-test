...
static const int AES_256_GCM_KEY_LEN = 32;
...
std::vector<uint8_t> encrypt(const uint8_t* key,
                             ...
                             int plaintext_len) {
    std::vector<uint8_t> iv(AES_256_GCM_IV_LEN);
    if (RAND_bytes(iv.data(), AES_256_GCM_IV_LEN) != 1) {
    ...
    std::vector<uint8_t> ciphertext(plaintext_len + AES_256_GCM_TAG_LEN);
    int out_len = 0;
    ...
    uint8_t tag[AES_256_GCM_TAG_LEN];
    ...
    result.reserve(AES_256_GCM_IV_LEN + out_len + AES_256_GCM_TAG_LEN);
    result.insert(result.end(), iv.begin(), iv.end());
    result.insert(result.end(), ciphertext.data(),
                  ciphertext.data() + out_len);
    result.insert(result.end(), tag, tag + AES_256_GCM_TAG_LEN);
    return result;
}
...
std::vector<uint8_t> decrypt(const uint8_t* key,
                             const uint8_t* data,
                             int data_len) {
    if (data_len < AES_256_GCM_IV_LEN + AES_256_GCM_TAG_LEN) {
        throw std::runtime_error("Ciphertext too short");
    }

    const uint8_t* iv = data;
    int ciphertext_len = data_len - AES_256_GCM_IV_LEN
                         - AES_256_GCM_TAG_LEN;
    const uint8_t* ciphertext = data + AES_256_GCM_IV_LEN;
    const uint8_t* tag = data + AES_256_GCM_IV_LEN + ciphertext_len;
