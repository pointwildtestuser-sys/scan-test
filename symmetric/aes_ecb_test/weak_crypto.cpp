...
std::vector<unsigned char> encrypt(
    ...
    if (RAND_bytes(nonce.data(), AES_GCM_NONCE_LEN) != 1) {
    ...
    std::memcpy(output.data(), nonce.data(), AES_GCM_NONCE_LEN);
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG,
            AES_GCM_TAG_LEN,
            output.data() + AES_GCM_NONCE_LEN + out_len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
...
std::vector<unsigned char> decrypt(
    ...
    int enc_len = ciphertext_len - AES_GCM_NONCE_LEN
                  - AES_GCM_TAG_LEN;
    const unsigned char* tag = ciphertext + AES_GCM_NONCE_LEN
                               + enc_len;
